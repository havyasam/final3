...
def predict(disease):
    ...
    svm_prob = svm_model.predict_proba(data)[0][1]  # Probability for class 1 (positive class)
    rf_prob = rf_model.predict_proba(data)[0][1]    # Probability for class 1 (positive class)
...
def get_accuracy(disease):
    if disease == 'diabetes':
        return jsonify({'SVM Accuracy': diabetes_svm_acc, 'Random Forest Accuracy': diabetes_rf_acc})
    elif disease == 'cardio':
        return jsonify({'SVM Accuracy': cardio_svm_acc, 'Random Forest Accuracy': cardio_rf_acc})
    elif disease == 'kidney':
        return jsonify({'SVM Accuracy': kidney_svm_acc, 'Random Forest Accuracy': kidney_rf_acc})
    else:
        return jsonify({'error': 'Invalid disease type'}), 400
